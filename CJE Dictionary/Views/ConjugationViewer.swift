...
struct ConjugationViewer: View {
    ...
    var body: some View {
        if let first = conjugatedVerbs.first {
            // I know this sucks, i just dont know how to make it detect overflow...
            let nextVerbForm = conjugatedVerbs.first(where: { $0.form != first.form })
            let nextVerbForm1 = conjugatedVerbs.first(where: { $0.form != first.form && $0.form != nextVerbForm?.form })
            HStack {
                VStack {
                ...
                if let thirdVerb = nextVerbForm1 {
                    Divider()
                    VStack {
                        Text(LocalizedStringKey(thirdVerb.form)).font(.subheadline)
                        Text(thirdVerb.verb)           .font(.caption)
                    }
                }
                Divider()
